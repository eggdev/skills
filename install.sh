...
# Skill installer: the `equip` command, runnable from any directory. The
# package's prepare script links the bin, so plain `npm install` wires it too.
(cd "$REPO" && npm install --no-fund --no-audit --silent)
